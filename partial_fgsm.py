...
class PartialFgsmAttack(LinfPGDAttack):
  def perturb(self, x_nat, y, sess, params):
    ...
    for i in range(self.k):
      ...
      # Flatten once to make 1024 (32x32).
      pre_flatten_1 = x_nat.shape
      grad = grad.reshape((x_nat.shape[0], x_nat.shape[1] * x_nat.shape[2], -1))
      x = x.reshape((x_nat.shape[0], x_nat.shape[1] * x_nat.shape[2], -1))

      # Flatten again if needed.
      if params['rgb_flatten']:
        pre_flatten_2 = x.shape
        grad = grad.reshape((-1, x_nat.shape[1] * x_nat.shape[2]))
        x = x.reshape((-1, x_nat.shape[1] * x_nat.shape[2]))
        # Force cast to ints for valid numbers.
        x = self._run_attack(x, grad, params).astype('int64')
        x = x.reshape(pre_flatten_2)
      else:
        # Non-flatten, so run on all 3 channels separately and concat.
        x_all = []
        for i in range(x_nat.shape[3]):
          # Force cast to ints for valid numbers.
          temp = self._run_attack(x[:, :, i], grad[:, :, i], params).astype('int64')
          x_all.append(temp)
        x = np.stack(x_all)
        assert x.shape == x_nat.shape

      x = x.reshape(pre_flatten_1)
